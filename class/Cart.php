...
class Cart extends Database implements iCart {
	...
	{
		$sql = "INSERT INTO cart(item_id, cart_qty, department, someone, cart_stock_id, user_id, cart_uniqid)
				VALUES(?,?,?,?,?,?,?)";
		return $this->insertRow($sql, [$item_id, $qty, $dep, $som, $stock_id, $user_id, $uniqid]);
	}//end add_toCart
	...
	$sql = "SELECT *
			...
			WHERE s.cart_id = ?";
	return $this->getRow($sql, [$cart_id]);
    }//end get_stocklist
